<script setup lang="ts">
import { computed, onMounted, ref } from 'vue'
import {
    CaretBottom,
    Sort,
    Location,
    PieChart,
    Clock,
    Money,
    Star,
    CaretTop,
} from '@element-plus/icons-vue'
import { useRoute } from 'vue-router'
import {
    getDeliveryMode,
    getOtherRestaurant,
    OtherRestaurantParams,
} from '@/apis/delivery-home.ts'
import { usePosition } from '@/store/position.ts'
import { ResCate, SingleRes } from '@/apis/types/dekivery-home.ts'
const route = useRoute()
const store = usePosition()
const title = route.params.title
const emit = defineEmits(['getValues'])

const category = ref(false)
const categoryShow = () => {
    sortShow.value = false
    filter.value = false
    category.value = !category.value
}
const sortShow = ref(false)
const showSortContent = () => {
    filter.value = false
    category.value = false
    sortShow.value = !sortShow.value
}
const filter = ref(false)
const showFilter = () => {
    sortShow.value = false
    category.value = false
    filter.value = !filter.value
}

//#region 获取店铺分类列表
const getResCateParams = ref<OtherRestaurantParams>({
    longitude: store.longitude,
    latitude: store.latitude,
} as OtherRestaurantParams)
const ResCateData = ref<ResCate>()
const getResCateApi = async () => {
    const res = await getOtherRestaurant(getResCateParams.value)
    ResCateData.value = res
    ResDetailCate.value = res[0]
}
const ResDetailCate = ref<SingleRes | any>({} as SingleRes | any)
const showDetailResCate = (name: string) => {
    ResDetailCate.value = ResCateData.value!.find((res) => {
        return res.name === name
    })
}

//endregion
//#region 分类筛选功能

const cate_id = ref<number | string>('')
const sort_id = ref<number | string>('')
const select_id = ref<number[]>([] as number[])

const categoryFilter = (id: number) => {
    //获取数据将数据传给父亲组件
    cate_id.value = id
    emit('getValues', id, sort_id, select_id)
    //之后这个弹窗消失
    category.value = false
}
//endregion

//#region 排序功能
const handledataset = (e: any) => {
    sort_id.value = e.target.dataset.id
    emit('getValues', cate_id.value, e.target.dataset.id, select_id)
    //弹窗消失
    sortShow.value = false
}
//endregion

//#region获取配送方式
const delivery_mode = ref<string>('')
const getDeliveryModeApi = async () => {
    const res = await getDeliveryMode({
        latitude: store.latitude,
        longitude: store.longitude,
    })
    delivery_mode.value = res[0].text
}
//endregion

//#region 筛选功能
const showSelect = ref(false)
const handleDeliverySelect = () => {
    showSelect.value = !showSelect.value
}
const handleBusinessSelect = (event: any) => {
    if (select_id.value.find((item) => item == event.target.dataset.id)) {
        select_id.value.splice(
            select_id.value.indexOf(event.target.dataset.id),
            1,
        )
        return
    }
    select_id.value.push(event.target.dataset.id)
    //将数组去重
    const newSet = new Set(select_id.value)
    select_id.value = Array.from(newSet)
}
const selectLength = computed(() => {
    return select_id.value.length + (showSelect.value ? 1 : 0)
})
const clearAll = () => {
    select_id.value = []
    showSelect.value = false
}
//确定
const handleConfirm = () => {
    if (select_id.value.length == 0) {
        filter.value = false
        return
    }
    emit('getValues', cate_id.value, sort_id.value, select_id.value)
    filter.value = false
}
//endregion
onMounted(() => {
    getResCateApi()
    getDeliveryModeApi()
})
</script>

<template>
    <div class="menu">
        <ul class="filter_ul">
            <li>
                <div @click="categoryShow">
                    <span>{{ title }}</span>
                    <el-icon v-if="category"><CaretBottom /></el-icon>
                    <el-icon v-else><CaretTop></CaretTop></el-icon>
                </div>
                <transition name="showlist" v-show="category">
                    <section class="pops cate">
                        <div class="left">
                            <ul>
                                <li
                                    v-for="item in ResCateData"
                                    :key="item.id"
                                    @click="showDetailResCate(item.name)"
                                >
                                    <span>{{ item.name }}</span>
                                    <span class="num">{{ item.count }}</span>
                                    <span>&gt;</span>
                                </li>
                            </ul>
                        </div>
                        <div class="right">
                            <ul>
                                <li
                                    v-for="item in ResDetailCate.sub_categories"
                                    :key="item.id"
                                    @click="categoryFilter(item.id)"
                                >
                                    <span>{{ item.name }}</span>
                                    <span class="num">{{ item.count }}</span>
                                </li>
                            </ul>
                        </div>
                    </section>
                </transition>
            </li>
            <li>
                <div @click="showSortContent">
                    <span>排序</span>
                    <el-icon v-if="sortShow"><CaretBottom /></el-icon>
                    <el-icon v-else><CaretTop /></el-icon>
                </div>
                <transition name="showlist">
                    <section class="pops" v-show="sortShow">
                        <ul class="sort_ul" @click="handledataset($event)">
                            <li data-id="4">
                                <el-icon color="#4B91CD"><Sort /></el-icon>
                                <span>智能排序</span>
                            </li>
                            <li data-id="5">
                                <el-icon color="#4B91CD"><Location /></el-icon>
                                <span>距离最近</span>
                            </li>
                            <li data-id="6">
                                <el-icon color="#F17B7B"><PieChart /></el-icon>
                                <span>销售最高</span>
                            </li>
                            <li data-id="1">
                                <el-icon color="#FDD835"><Money /></el-icon>
                                <span>起送价最低</span>
                            </li>
                            <li data-id="2">
                                <el-icon color="#B4EAE4"><Clock /></el-icon>
                                <span>配送速度最快</span>
                            </li>
                            <li data-id="3">
                                <el-icon color="#FDD835"><Star /></el-icon>
                                <span>配送速度最快</span>
                            </li>
                        </ul>
                    </section>
                </transition>
            </li>
            <li>
                <div @click="showFilter">
                    <span>筛选</span>
                    <el-icon v-if="filter"><CaretBottom /></el-icon>
                    <el-icon v-else><CaretTop /></el-icon>
                </div>
                <transition name="showlist">
                    <section class="pops filter_detail" v-show="filter">
                        <div
                            class="delivery_method"
                            @click="handleDeliverySelect"
                        >
                            <span class="title">配送方式</span>
                            <div :class="{ btn: true, active: showSelect }">
                                <span v-if="!showSelect">
                                    <svg
                                        t="1703086922524"
                                        class="icon bird"
                                        viewBox="0 0 1024 1024"
                                        version="1.1"
                                        xmlns="http://www.w3.org/2000/svg"
                                        p-id="794"
                                        xmlns:xlink="http://www.w3.org/1999/xlink"
                                        width="200"
                                        height="200"
                                    >
                                        <path
                                            d="M785.24 152.477c-267.89 0-208.961 176.177-208.961 176.177-36.352-11.115-491.85-161.742-556.775-183.097a2.098 2.098 0 0 0-2.114 3.303c6.606 7.829 19.852 22.858 41.19 46.311a11.726 11.726 0 0 0 6.013 3.914l6.325 2.395 378.814 123.871a8.82 8.82 0 0 1 5.12 4.212l20.728 34.882-368.888-120.27 27.35 30.059a21.741 21.741 0 0 0 9.333 6.028L493.615 395.1a9.396 9.396 0 0 1 5.12 3.914l18.036 35.477-336.12-108.528 30.06 33.066a11.726 11.726 0 0 0 4.806 3.006l315.69 103.424a11.77 11.77 0 0 1 6.92 5.714l20.15 33.677-293.74-96.207c-1.503 0-2.709 1.487-1.503 2.395 65.536 65.552 123.87 117.562 145.804 120.27l-210.465 474.426a3.303 3.303 0 0 0 5.12 3.914c94.687-77.873 161.743-285.018 262.442-340.05C612.069 589.923 668.29 543.034 704.36 463.36c42.397-94.109 96.504-251.359 106.133-257.652 57.427-44.51 160.834-144.35 204.437-187.045a3.617 3.617 0 1 0-5.12-5.103c-60.73 39.688-216.163 138.9-224.57 138.9z"
                                            p-id="795"
                                        ></path>
                                    </svg>
                                </span>
                                <span v-else>
                                    <svg
                                        t="1703168967336"
                                        class="icon selected"
                                        viewBox="0 0 1300 1024"
                                        version="1.1"
                                        xmlns="http://www.w3.org/2000/svg"
                                        p-id="4974"
                                        width="20"
                                        height="20"
                                    >
                                        <path
                                            d="M1230.490183 89.988301h-101.410965a46.247857 46.247857 0 0 0-36.403936 17.644765L494.332018 865.522151 207.465288 502.039995a46.433592 46.433592 0 0 0-36.403936-17.644765H69.650388a11.608398 11.608398 0 0 0-9.100984 18.666304l397.471546 503.525869c18.573437 23.495397 54.234435 23.495397 72.900739 0l708.669478-898.025665a11.515531 11.515531 0 0 0-9.100984-18.573437z"
                                            fill="#d81e06"
                                            fill-opacity=".65"
                                            p-id="4975"
                                        ></path>
                                    </svg>
                                </span>
                                <span style="font-size: 1vw">{{
                                    delivery_mode
                                }}</span>
                            </div>
                        </div>
                        <div class="business_attr">
                            <span class="title">商家属性(可以多选)</span>
                            <div class="business_attr_btns">
                                <div
                                    :data-id="0"
                                    @click="handleBusinessSelect($event)"
                                    :class="{
                                        btn: true,
                                        active: select_id.find(
                                            (item) => item == 0,
                                        ),
                                    }"
                                >
                                    <span
                                        v-if="
                                            !select_id.find((item) => item == 0)
                                        "
                                    >
                                        <svg
                                            t="1703090529142"
                                            class="icon pin"
                                            viewBox="0 0 1024 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="1869"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M981.333333 0 42.666667 0 0 0l0 42.666667 0 938.666667 0 42.666667 42.666667 0 938.666667 0 42.666667 0 0-42.666667L1024 42.666667 1024 0 981.333333 0zM981.333333 981.333333 42.666667 981.333333 42.666667 42.666667l938.666667 0L981.333333 981.333333z"
                                                fill="#FF7300"
                                                p-id="1870"
                                            ></path>
                                            <path
                                                d="M661.333333 469.333333l42.666667 0 0-42.666667 0-128 0-42.666667-42.666667 0L362.666667 256l-42.666667 0 0 42.666667 0 128 0 42.666667 42.666667 0L661.333333 469.333333zM362.666667 298.666667l298.666667 0 0 128L362.666667 426.666667 362.666667 298.666667z"
                                                fill="#FF7300"
                                                p-id="1871"
                                            ></path>
                                            <path
                                                d="M533.333333 512l0 42.666667 0 149.333333 0 42.666667 42.666667 0 149.333333 0 42.666667 0 0-42.666667 0-149.333333 0-42.666667-42.666667 0-149.333333 0L533.333333 512zM725.333333 554.666667l0 149.333333-149.333333 0 0-149.333333L725.333333 554.666667z"
                                                fill="#FF7300"
                                                p-id="1872"
                                            ></path>
                                            <path
                                                d="M298.666667 746.666667l149.333333 0 42.666667 0 0-42.666667 0-149.333333 0-42.666667-42.666667 0-149.333333 0-42.666667 0 0 42.666667 0 149.333333 0 42.666667L298.666667 746.666667zM298.666667 554.666667l149.333333 0 0 149.333333-149.333333 0L298.666667 554.666667z"
                                                fill="#FF7300"
                                                p-id="1873"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span v-else>
                                        <svg
                                            t="1703168967336"
                                            class="icon selected"
                                            viewBox="0 0 1300 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="4974"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M1230.490183 89.988301h-101.410965a46.247857 46.247857 0 0 0-36.403936 17.644765L494.332018 865.522151 207.465288 502.039995a46.433592 46.433592 0 0 0-36.403936-17.644765H69.650388a11.608398 11.608398 0 0 0-9.100984 18.666304l397.471546 503.525869c18.573437 23.495397 54.234435 23.495397 72.900739 0l708.669478-898.025665a11.515531 11.515531 0 0 0-9.100984-18.573437z"
                                                fill="#d81e06"
                                                fill-opacity=".65"
                                                p-id="4975"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span style="font-size: 1vw">品牌商家</span>
                                </div>
                                <div
                                    :data-id="1"
                                    @click="handleBusinessSelect($event)"
                                    :class="{
                                        btn: true,
                                        active: select_id.find(
                                            (item) => item == 1,
                                        ),
                                    }"
                                >
                                    <span
                                        v-if="
                                            !select_id.find((item) => item == 1)
                                        "
                                    >
                                        <svg
                                            t="1703090217600"
                                            class="icon bao"
                                            viewBox="0 0 1024 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="1125"
                                            xmlns:xlink="http://www.w3.org/1999/xlink"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M512 0C509.22737 0 233.998135 0.744223 0.474686 62.806013l0 391.878616 0 56.266976 0 0.555059c0.426285 5.818792 7.917359 93.944367 71.873454 198.890444 38.799054 63.68345 90.436396 120.723068 153.473313 169.588668 78.332562 60.735866 174.603736 108.920298 286.177659 143.20961 111.247992-34.224481 207.295367-82.192219 285.488498-142.665208 62.998729-48.715512 114.646728-105.606819 153.524823-169.071798 64.377051-105.154779 72.076827-194.23772 72.511993-199.957933l0-0.555059 0-56.266976L1023.524426 62.806013C790.008081 0.744223 514.783287 0 512 0L512 0zM980.89147 454.684629l0 54.971247c-1.121663 11.583411-10.407574 88.34227-66.23672 179.533546-36.173848 59.059144-84.369825 112.091682-143.250462 157.618048-71.129231 55.015652-158.355167 99.290694-259.414056 131.706128-101.271144-32.459838-188.700453-76.903618-260.048155-132.21767C192.992168 800.596383 144.816616 747.411093 108.751116 688.213406 53.085379 596.86849 44.159146 521.011047 43.107642 509.730476l0-55.045847L43.107642 96.028724C262.681798 43.616964 508.546202 42.640061 511.999112 42.634732c3.447581 0.005329 249.31465 0.976903 468.89147 53.393992l0 358.655904L980.89147 454.684629z"
                                                fill="#cdcdcd"
                                                p-id="1126"
                                            ></path>
                                            <path
                                                d="M342.969055 189.08855c-17.927067 73.896532-37.434053 129.40774-83.218855 191.871837 0 14.192631 0 51.956168 0 78.894725 30.397684-36.690719 49.512133-58.388633 64.44277-87.874244l0 328.753777 42.689794 0 0-412.53746c9.666904-29.702306 16.259226-55.641758 21.61621-99.109523L342.969055 189.08855 342.969055 189.08855z"
                                                fill="#cdcdcd"
                                                p-id="1127"
                                            ></path>
                                            <path
                                                d="M772.431811 380.812075 772.431811 189.203114 473.387446 189.203114l0 191.608961 128.214142 0 0 64.185223-192.254605 0 0 43.264391 151.899611 0c-38.49355 59.944574-64.484511 90.530534-130.121768 125.420198l20.907511 39.105446c66.818422-35.04064 122.691085-94.845783 149.568364-161.234368l0 209.206545 41.985536 0L643.586235 491.553854c30.708517 67.039557 79.319234 122.663554 146.355239 157.554994l24.768944-38.880759c-58.640852-27.924342-95.186811-64.693213-130.090684-121.964624l151.885401 0 0-43.264391L643.586235 444.999075l0-64.185223 128.844688 0L772.431811 380.812075 772.431811 380.812075zM515.761078 338.553895 515.761078 232.096282l214.205627 0 0 106.457613L515.761078 338.553895z"
                                                fill="#cdcdcd"
                                                p-id="1128"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span v-else>
                                        <svg
                                            t="1703168967336"
                                            class="icon selected"
                                            viewBox="0 0 1300 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="4974"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M1230.490183 89.988301h-101.410965a46.247857 46.247857 0 0 0-36.403936 17.644765L494.332018 865.522151 207.465288 502.039995a46.433592 46.433592 0 0 0-36.403936-17.644765H69.650388a11.608398 11.608398 0 0 0-9.100984 18.666304l397.471546 503.525869c18.573437 23.495397 54.234435 23.495397 72.900739 0l708.669478-898.025665a11.515531 11.515531 0 0 0-9.100984-18.573437z"
                                                fill="#d81e06"
                                                fill-opacity=".65"
                                                p-id="4975"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span style="font-size: 1vw">外卖保</span>
                                </div>
                                <div
                                    :data-id="9"
                                    @click="handleBusinessSelect($event)"
                                    :class="{
                                        btn: true,
                                        active: select_id.find(
                                            (item) => item == 9,
                                        ),
                                    }"
                                >
                                    <span
                                        v-if="
                                            !select_id.find((item) => item == 9)
                                        "
                                    >
                                        <svg
                                            t="1703090757836"
                                            class="icon zhun"
                                            viewBox="0 0 1024 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="2015"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M602.965333 882.176A381.098667 381.098667 0 1 1 512 130.901333a382.976 382.976 0 0 1 376.832 324.266667 17.066667 17.066667 0 0 0 19.456 14.336 17.066667 17.066667 0 0 0 14.336-19.456A415.232 415.232 0 1 0 512 927.232a420.181333 420.181333 0 0 0 99.157333-11.776 17.066667 17.066667 0 0 0 12.458667-20.650667 17.066667 17.066667 0 0 0-20.650667-12.629333z"
                                                fill="#1296db"
                                                p-id="2016"
                                            ></path>
                                            <path
                                                d="M543.402667 732.330667a17.066667 17.066667 0 1 0 0-34.133334h-62.805334a17.066667 17.066667 0 1 0 0 34.133334z"
                                                fill="#1296db"
                                                p-id="2017"
                                            ></path>
                                            <path
                                                d="M603.989333 816.810667a317.44 317.44 0 0 1-74.922666 12.8v-24.234667a17.066667 17.066667 0 0 0-34.133334 0v24.064a316.074667 316.074667 0 0 1-126.976-34.133333l12.117334-20.821334a17.066667 17.066667 0 1 0-29.525334-17.066666l-12.117333 20.821333a320.853333 320.853333 0 0 1-92.842667-92.842667l20.821334-12.117333a17.066667 17.066667 0 1 0-17.066667-29.525333l-20.821333 12.117333a316.074667 316.074667 0 0 1-34.133334-126.976h24.064a17.066667 17.066667 0 1 0 0-34.133333h-23.893333a316.074667 316.074667 0 0 1 34.133333-126.976l20.821334 12.117333a17.066667 17.066667 0 1 0 17.066666-29.525333l-20.821333-12.117334a320.853333 320.853333 0 0 1 92.842667-92.842666l12.117333 20.821333a17.066667 17.066667 0 1 0 29.525333-17.066667l-12.117333-20.821333a316.074667 316.074667 0 0 1 126.976-34.133333v24.064a17.066667 17.066667 0 1 0 34.133333 0V194.56a314.88 314.88 0 0 1 126.976 34.133333l-12.117333 20.821334a17.066667 17.066667 0 1 0 29.525333 17.066666l12.117334-20.821333a319.317333 319.317333 0 0 1 93.013333 92.842667l-20.992 12.117333a17.066667 17.066667 0 1 0 17.066667 29.525333l20.992-12.117333a312.832 312.832 0 0 1 29.013333 84.138667 17.066667 17.066667 0 1 0 34.133333-6.314667A352.256 352.256 0 1 0 614.4 849.408a17.066667 17.066667 0 0 0 11.434667-21.162667 17.066667 17.066667 0 0 0-21.845334-11.434666z"
                                                fill="#1296db"
                                                p-id="2018"
                                            ></path>
                                            <path
                                                d="M512 491.008A20.992 20.992 0 1 0 532.992 512 20.992 20.992 0 0 0 512 491.008z"
                                                fill="#1296db"
                                                p-id="2019"
                                            ></path>
                                            <path
                                                d="M565.76 571.050667A80.042667 80.042667 0 0 0 592.213333 512a74.410667 74.410667 0 0 0-1.194666-10.581333l110.08-109.909334a48.469333 48.469333 0 1 0-68.266667-68.266666l-109.909333 110.08a74.410667 74.410667 0 0 0-10.922667-1.536 75.605333 75.605333 0 0 0-21.504 3.413333l-91.648-69.12A48.469333 48.469333 0 1 0 341.333333 443.733333l91.477334 68.266667A80.213333 80.213333 0 0 0 512 592.213333a76.117333 76.117333 0 0 0 24.234667-4.266666l78.336 136.533333a17.066667 17.066667 0 0 0 29.525333-17.066667z m90.794667-223.914667a14.336 14.336 0 0 1 20.309333 20.309333l-98.986667 98.986667a82.090667 82.090667 0 0 0-20.309333-20.309333zM440.661333 476.16l-79.530666-59.904a14.336 14.336 0 1 1 17.066666-22.869333l79.530667 59.904a83.968 83.968 0 0 0-17.066667 22.869333zM512 558.08a46.08 46.08 0 1 1 46.08-46.08 46.08 46.08 0 0 1-46.08 46.08z"
                                                fill="#1296db"
                                                p-id="2020"
                                            ></path>
                                            <path
                                                d="M892.586667 646.314667v-30.72a34.133333 34.133333 0 0 0 34.133333-34.133334v-39.765333a17.066667 17.066667 0 0 0-17.066667-17.066667H681.642667a17.066667 17.066667 0 0 0-17.066667 17.066667v39.765333a34.133333 34.133333 0 0 0 34.133333 34.133334v30.72a96.597333 96.597333 0 0 0 41.984 79.530666A97.109333 97.109333 0 0 0 699.733333 805.546667V836.266667a34.133333 34.133333 0 0 0-34.133333 34.133333v39.765333a17.066667 17.066667 0 0 0 17.066667 17.066667h227.498666a17.066667 17.066667 0 0 0 17.066667-17.066667V870.4a34.133333 34.133333 0 0 0-34.133333-34.133333v-30.72a97.109333 97.109333 0 0 0-41.813334-79.701334 96.426667 96.426667 0 0 0 41.301334-79.530666z m-193.877334-64.853334v-22.698666h194.389334v22.698666zM893.098667 870.4v22.698667H698.709333V870.4z m-34.133334-64.853333V836.266667H733.866667v-30.72a62.634667 62.634667 0 1 1 125.098666 0z m0-159.232a62.634667 62.634667 0 0 1-125.098666 0v-30.72h124.586666z"
                                                fill="#1296db"
                                                p-id="2021"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span v-else>
                                        <svg
                                            t="1703168967336"
                                            class="icon selected"
                                            viewBox="0 0 1300 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="4974"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M1230.490183 89.988301h-101.410965a46.247857 46.247857 0 0 0-36.403936 17.644765L494.332018 865.522151 207.465288 502.039995a46.433592 46.433592 0 0 0-36.403936-17.644765H69.650388a11.608398 11.608398 0 0 0-9.100984 18.666304l397.471546 503.525869c18.573437 23.495397 54.234435 23.495397 72.900739 0l708.669478-898.025665a11.515531 11.515531 0 0 0-9.100984-18.573437z"
                                                fill="#d81e06"
                                                fill-opacity=".65"
                                                p-id="4975"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span style="font-size: 1vw">准时达</span>
                                </div>
                                <div
                                    :data-id="3"
                                    @click="handleBusinessSelect($event)"
                                    :class="{
                                        btn: true,
                                        active: select_id.find(
                                            (item) => item == 3,
                                        ),
                                    }"
                                >
                                    <span
                                        v-if="
                                            !select_id.find((item) => item == 3)
                                        "
                                    >
                                        <svg
                                            t="1703090825859"
                                            class="icon xin"
                                            viewBox="0 0 1024 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="2315"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M508.330667 733.994667c-11.008-7.338667-13.44-17.109333-7.338667-29.333334 28.117333-37.888 41.557333-98.986667 40.341333-183.317333v-165.013333c0-14.656 7.338667-23.210667 21.994667-25.664 37.888-1.216 82.496-5.504 133.845333-12.842667 13.44-2.432 21.376 3.072 23.829334 16.512 1.216 12.224-4.266667 19.562667-16.512 21.994667a1787.093333 1787.093333 0 0 1-113.664 11.008c-6.101333 0-9.173333 3.669333-9.173334 10.986666v84.330667h135.68c12.224 1.237333 18.944 7.957333 20.16 20.181333-1.216 10.986667-7.936 17.109333-20.16 18.346667h-36.672v223.658667c-1.216 12.202667-7.936 18.944-20.16 20.16-11.008-1.216-17.109333-7.957333-18.346666-20.16V501.162667h-60.48v18.346666c1.216 92.885333-13.44 161.92-44.010667 207.146667-6.101333 12.224-15.893333 14.677333-29.333333 7.338667z m-131.989334-282.325334c-1.237333 0-2.453333 0.618667-3.669333 1.834667h45.824a522.666667 522.666667 0 0 0 16.512-31.168c7.317333-12.224 12.224-20.778667 14.656-25.664 6.122667-11.008 15.274667-14.677333 27.52-11.008 9.770667 6.122667 12.202667 14.058667 7.317333 23.829333-4.906667 9.792-13.44 24.448-25.664 44.010667h49.493334c9.770667 1.216 15.274667 6.72 16.512 16.490667-1.237333 11.008-6.741333 17.109333-16.512 18.346666h-82.496a12.437333 12.437333 0 0 1 3.669333 9.173334v38.485333h69.653333c9.792 1.216 15.296 6.72 16.512 16.490667-1.216 11.008-6.72 17.130667-16.512 18.346666h-69.653333v108.16c0 34.218667-15.274667 51.946667-45.845333 53.162667h-16.490667a195.157333 195.157333 0 0 1-20.16 1.834667c-12.224 0-19.562667-6.72-22.016-20.16 1.237333-12.224 7.338667-18.944 18.346667-20.16 2.432 0 6.101333 0.597333 10.986666 1.834666h11.008c15.893333 0 23.829333-8.554667 23.829334-25.685333v-98.986667H314.026667c-11.008-1.216-17.109333-7.338667-18.346667-18.346666 1.237333-9.770667 7.338667-15.274667 18.346667-16.490667h75.157333V497.493333c0-3.669333 1.216-6.72 3.669333-9.173333h-89.813333c-11.029333-1.216-17.130667-7.317333-18.346667-18.325333 1.216-9.770667 7.317333-15.274667 18.346667-16.490667h56.810667c-3.669333-1.216-6.72-4.266667-9.173334-9.173333-1.216-1.216-3.050667-4.266667-5.482666-9.173334a758.336 758.336 0 0 0-14.677334-23.829333c-4.885333-9.770667-3.050667-17.706667 5.504-23.829333 11.008-3.669333 19.562667-1.216 25.664 7.338666 2.453333 2.432 6.122667 7.338667 11.008 14.656 6.101333 8.554667 9.770667 14.08 10.986667 16.512 4.906667 9.770667 2.453333 18.346667-7.317333 25.664z m-60.501333-71.509333c-9.792-1.216-15.274667-7.317333-16.512-18.346667 1.237333-9.749333 6.72-15.253333 16.512-16.490666h75.157333c-3.669333-12.202667-7.338667-21.973333-10.986666-29.333334-1.237333-12.202667 3.648-19.541333 14.656-21.973333 12.224-2.453333 21.397333 1.216 27.52 10.986667 0 1.216 0.597333 3.669333 1.813333 7.338666 4.906667 15.872 9.173333 26.88 12.842667 32.981334h60.48c11.008 1.237333 17.130667 6.741333 18.346666 16.512-1.216 11.008-7.338667 17.109333-18.346666 18.346666h-181.482667z m-14.677333 311.68c-8.533333-6.122667-10.986667-14.08-7.338667-23.829333a1659.648 1659.648 0 0 0 33.002667-66.005334c4.906667-9.792 12.224-12.842667 22.016-9.173333 9.770667 4.906667 13.44 12.224 10.986666 21.994667-3.669333 6.122667-9.173333 17.728-16.490666 34.837333-8.554667 15.893333-14.677333 27.52-18.346667 34.837333-4.885333 8.554667-12.821333 11.008-23.829333 7.338667z m201.664-25.664c-9.770667 4.885333-18.346667 2.432-25.664-7.338667a1138.56 1138.56 0 0 1-27.498667-44.010666c-4.885333-8.533333-3.050667-16.490667 5.504-23.829334 9.770667-3.669333 18.346667-1.216 25.664 7.338667l14.677333 21.994667c6.101333 9.770667 10.389333 17.109333 12.821334 21.994666 4.906667 8.554667 3.050667 16.512-5.504 23.850667z"
                                                fill="#db5544"
                                                p-id="2316"
                                            ></path>
                                            <path
                                                d="M675.328 117.717333A425.429333 425.429333 0 0 0 512 85.333333C276.352 85.333333 85.333333 276.352 85.333333 512s191.018667 426.666667 426.666667 426.666667 426.666667-191.018667 426.666667-426.666667c0-56.746667-11.093333-112-32.384-163.328a21.333333 21.333333 0 0 0-39.402667 16.341333A382.762667 382.762667 0 0 1 896 512c0 212.074667-171.925333 384-384 384S128 724.074667 128 512 299.925333 128 512 128c51.114667 0 100.8 9.984 146.986667 29.12a21.333333 21.333333 0 0 0 16.341333-39.402667z"
                                                fill="#db5544"
                                                p-id="2317"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span v-else>
                                        <svg
                                            t="1703168967336"
                                            class="icon selected"
                                            viewBox="0 0 1300 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="4974"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M1230.490183 89.988301h-101.410965a46.247857 46.247857 0 0 0-36.403936 17.644765L494.332018 865.522151 207.465288 502.039995a46.433592 46.433592 0 0 0-36.403936-17.644765H69.650388a11.608398 11.608398 0 0 0-9.100984 18.666304l397.471546 503.525869c18.573437 23.495397 54.234435 23.495397 72.900739 0l708.669478-898.025665a11.515531 11.515531 0 0 0-9.100984-18.573437z"
                                                fill="#d81e06"
                                                fill-opacity=".65"
                                                p-id="4975"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span style="font-size: 1vw">新店</span>
                                </div>
                                <div
                                    :data-id="5"
                                    @click="handleBusinessSelect($event)"
                                    :class="{
                                        btn: true,
                                        active: select_id.find(
                                            (item) => item == 5,
                                        ),
                                    }"
                                >
                                    <span
                                        v-if="
                                            !select_id.find((item) => item == 5)
                                        "
                                    >
                                        <svg
                                            t="1703090924984"
                                            class="icon fu"
                                            viewBox="0 0 1024 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="2623"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M173.077333 362.666667l91.114667-214.677334a65.6 65.6 0 0 1 86.016-34.773333c11.584 4.906667 24.96 10.282667 40.896 16.448 8.277333 3.2 16.789333 6.464 27.904 10.666667 28.202667 10.709333 39.296 14.933333 46.144 17.642666l51.477333-51.669333c28.181333-28.16 74.112-27.946667 102.570667 0.533333l195.925333 195.925334c16.426667 16.426667 23.445333 38.634667 21.056 59.904H896a42.666667 42.666667 0 0 1 42.666667 42.666666v490.666667a42.666667 42.666667 0 0 1-42.666667 42.666667H128a42.666667 42.666667 0 0 1-42.666667-42.666667V405.333333a42.666667 42.666667 0 0 1 42.666667-42.666666h45.077333z m48.96 0h39.104l169.194667-169.770667-27.328-10.389333c-11.2-4.245333-19.818667-7.530667-28.224-10.794667a1459.2 1459.2 0 0 1-42.197333-17.002667 20.522667 20.522667 0 0 0-26.901334 10.88L222.037333 362.666667z m108.842667 0h454.954667a23.509333 23.509333 0 0 0-5.290667-25.322667l-195.925333-195.925333a23.36 23.36 0 0 0-33.024-0.213334L330.88 362.666667zM128 405.333333v490.666667h768V405.333333H128z m597.333333 320a85.333333 85.333333 0 1 1 0-170.666666 85.333333 85.333333 0 0 1 0 170.666666z m0-42.666666a42.666667 42.666667 0 1 0 0-85.333334 42.666667 42.666667 0 0 0 0 85.333334z"
                                                fill="#d81e06"
                                                p-id="2624"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span v-else>
                                        <svg
                                            t="1703168967336"
                                            class="icon selected"
                                            viewBox="0 0 1300 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="4974"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M1230.490183 89.988301h-101.410965a46.247857 46.247857 0 0 0-36.403936 17.644765L494.332018 865.522151 207.465288 502.039995a46.433592 46.433592 0 0 0-36.403936-17.644765H69.650388a11.608398 11.608398 0 0 0-9.100984 18.666304l397.471546 503.525869c18.573437 23.495397 54.234435 23.495397 72.900739 0l708.669478-898.025665a11.515531 11.515531 0 0 0-9.100984-18.573437z"
                                                fill="#d81e06"
                                                fill-opacity=".65"
                                                p-id="4975"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span style="font-size: 1vw">在线支付</span>
                                </div>
                                <div
                                    :data-id="4"
                                    @click="handleBusinessSelect($event)"
                                    :class="{
                                        btn: true,
                                        active: select_id.find(
                                            (item) => item == 4,
                                        ),
                                    }"
                                >
                                    <span
                                        v-if="
                                            !select_id.find((item) => item == 4)
                                        "
                                    >
                                        <svg
                                            t="1703091009028"
                                            class="icon piao"
                                            viewBox="0 0 1024 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="2856"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M128 853.333333c0 0.064 768.064 0 768.064 0L896 170.666667H127.936L128 853.333333zM127.936 128h768.128C919.594667 128 938.666667 146.986667 938.666667 170.666667v682.666666c0 23.573333-19.029333 42.666667-42.602667 42.666667H127.936A42.56 42.56 0 0 1 85.333333 853.333333V170.666667c0-23.573333 19.029333-42.666667 42.602667-42.666667zM362.666667 298.666667v42.666666h298.666666v-42.666666H362.666667z m448 149.333333H213.333333v42.666667h597.333334v-42.666667z m-341.333334 128H213.333333v42.666667h256v-42.666667z m0 128H213.333333v42.666667h256v-42.666667z"
                                                fill="#8a8a8a"
                                                p-id="2857"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span v-else>
                                        <svg
                                            t="1703168967336"
                                            class="icon selected"
                                            viewBox="0 0 1300 1024"
                                            version="1.1"
                                            xmlns="http://www.w3.org/2000/svg"
                                            p-id="4974"
                                            width="20"
                                            height="20"
                                        >
                                            <path
                                                d="M1230.490183 89.988301h-101.410965a46.247857 46.247857 0 0 0-36.403936 17.644765L494.332018 865.522151 207.465288 502.039995a46.433592 46.433592 0 0 0-36.403936-17.644765H69.650388a11.608398 11.608398 0 0 0-9.100984 18.666304l397.471546 503.525869c18.573437 23.495397 54.234435 23.495397 72.900739 0l708.669478-898.025665a11.515531 11.515531 0 0 0-9.100984-18.573437z"
                                                fill="#d81e06"
                                                fill-opacity=".65"
                                                p-id="4975"
                                            ></path>
                                        </svg>
                                    </span>
                                    <span style="font-size: 1vw">开发票</span>
                                </div>
                            </div>
                        </div>
                        <div class="operation">
                            <button class="clear" @click="clearAll">
                                清空
                            </button>
                            <button class="confirm">
                                <span @click="handleConfirm">确定</span>
                                <span v-if="selectLength !== 0"
                                    >({{ selectLength }})</span
                                >
                            </button>
                        </div>
                    </section>
                </transition>
            </li>
        </ul>
    </div>
    <transition name="back_cover">
        <div class="back_cover" v-show="filter || category || sortShow"></div>
    </transition>
</template>
<style scoped lang="scss">
.menu {
}
.filter_ul {
    margin: 0;
    padding: 4vw 0;
    display: flex;
    background-color: white;
    li {
        border-right: 1px solid rgba(115, 115, 115, 0.3);
        flex: 1;
        text-align: center;
        &:last-child {
            border-right: 0;
        }
    }
}

//动画
.showlist-enter-active,
.showlist-leave-active {
    transition: all 0.3s;
    transform: translateY(0);
}
.showlist-enter,
.showlist-leave-active {
    opacity: 0;
    transform: translateY(0%);
}
.back_cover-enter-active,
.back_cover_leave-active {
    display: none;
}
.back_cover-enter,
.back_cover_leave-active {
    display: block;
}
//遮罩层
.pops {
    position: fixed;
    left: 0;
    top: 18.6vw;
    margin-top: 3vw;
    background-color: white;
    z-index: 9999;
}
//分类
.cate {
    width: 100%;
    display: flex;
    padding: 0;
    height: 80vw;
    font-size: 2vw;
    .left {
        width: 50%;
        background-color: rgba(115, 115, 115, 0.2);
        line-height: 1.5;
        height: 100%;
        overflow: auto;

        ul {
            padding: 0;
            li {
                padding: 2vw;
                text-align: left;
                display: flex;
                justify-content: space-between;
                span {
                    &:first-child {
                        flex: 1;
                    }
                }
                .num {
                    margin-right: 1vw;
                    padding: 0.5vw;
                    border-radius: 2vw;
                    background-color: rgba(115, 115, 115, 0.3);
                    font-size: 1vw;
                }
            }
        }
    }
    .right {
        flex: 1;
        overflow: auto;
        ul {
            padding: 2vw;
            li {
                padding: 2vw 0;
                border-right: 0;
                border-bottom: 1px solid rgba(115, 115, 115, 0.2);
                display: flex;
                justify-content: space-between;
                text-align: left;
                line-height: 1.5;
                &:last-child {
                    border-bottom: 0;
                }
            }
        }
    }
}
//排序
.sort_ul {
    padding: 0;
    margin: 0;
    li {
        border-top: 1px solid rgba(115, 115, 115, 0.2);
        padding: 4vw 3vw;
        display: flex;
        align-items: center;
        font-size: 3.8vw;
        width: 100vw;
        .el-icon {
            margin-right: 2vw;
        }
    }
}
.filter_detail {
    width: 100%;
    text-align: left;
    .delivery_method {
        padding: 0 2vw 1vw 2vw;
        .title {
            display: inline-block;
            margin-bottom: 2vw;
            font-size: 1vw;
        }
    }
    .btn {
        margin-bottom: 1vw;
        padding: 1.7vw 4vw 1.7vw 0;
        display: flex;
        align-items: center;
        width: 23%;
        border: 1px solid rgba(115, 115, 115, 0.1);
    }
    .business_attr {
        padding: 0 2vw 1vw 2vw;
        .title {
            font-size: 1vw;
        }
        .business_attr_btns {
            margin-top: 2vw;
            display: grid;
            grid-template-columns: 1fr 1fr 1fr;
            .btn {
                width: 70%;
                span {
                    pointer-events: none;
                }
            }
        }
    }
    .operation {
        padding: 2vw;
        display: flex;
        background-color: rgba(193, 193, 193, 0.3);
        button {
            padding: 3vw;
            border: 0;
        }
        .clear {
            width: 48%;
            margin-right: 2vw;
            background-color: rgba(255, 255, 255, 0.9);
            font-size: 3.9vw;
        }
        .confirm {
            flex: 1;
            background-color: $themeColor;
            color: whitesmoke;
            font-size: 3.9vw;
        }
    }
}
//svg
.bird {
    width: 4vw !important;
    height: 3vw !important;
    fill: $themeColor;
}
.bao {
    padding: 0 1vw;
}
.zhun {
    padding: 0 1vw;
}
.xin {
    padding: 0 1vw;
}
.fu {
    padding: 0 1vw;
}
.piao {
    padding: 0 1vw;
}

.pin {
    padding: 0 1vw;
}
//背景遮罩层
.back_cover {
    position: fixed;
    width: 100%;
    height: 100%;
    background-color: rgba(0, 0, 0, 0.3);
}
.active {
    color: $themeColor;
    border: 1px solid $themeColor !important;
    border-radius: 1vw;
}
.selected {
    fill: $themeColor;
}
</style>
